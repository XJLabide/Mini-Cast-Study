      ...
      {error ? <ErrorState message={error} /> : products.length === 0 ? <Loading /> : <>
        ...
        <section className="mt-10 flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-line bg-panel md:mt-12">
          <div className="flex flex-col gap-3 border-b border-line p-4 md:flex-row md:items-center md:justify-between md:p-5">
            <div className="flex h-11 min-w-0 flex-1 items-center gap-3 rounded-lg border border-line bg-ink px-3 text-muted md:max-w-xl"><IconSearch size={18} className="shrink-0" /><input aria-label="Search reorder products" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, category, or supplier" className="w-full min-w-0 bg-transparent text-sm text-copy outline-none placeholder:text-muted" /></div>
            <div className="flex w-full gap-3 sm:w-auto"><select aria-label="Filter by category" value={category} onChange={(event) => setCategory(event.target.value)} className="h-11 min-w-0 flex-1 rounded-lg border border-line bg-ink px-3 text-sm leading-5 text-copy outline-none focus:border-lime"><option value="all">All categories</option>{categories.map((item) => <option key={item}>{item}</option>)}</select><select aria-label="Filter by supplier" value={supplier} onChange={(event) => setSupplier(event.target.value)} className="h-11 min-w-0 flex-1 rounded-lg border border-line bg-ink px-3 text-sm leading-5 text-copy outline-none focus:border-lime"><option value="all">All suppliers</option>{suppliers.map((item) => <option key={item}>{item}</option>)}</select></div>
          </div>
